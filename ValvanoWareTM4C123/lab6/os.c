...
// vector number 51, interrupt number 35
  ...
  if(PeriodicTask1 == 0){
    ...
    PeriodicTask1_Init(period, priority);
    PeriodicTaskPeriod1 = period;
    ENABLE_PERIODICTASK1();    // 10) enable TIMER3A
    return 0;
  } else if(PeriodicTask2 == 0){
    PeriodicTask2 = task;
    PeriodicTask2_Init(period, priority);
    PeriodicTaskPeriod2 = period;
    ENABLE_PERIODICTASK2();    // 10) enable TIMER3A
    return 0;
  } else {
    return 1;
  }
...
void Timer5B_Handler(void){
  TIMER5_ICR_R |= TIMER_ICR_TBTOCINT;// acknowledge TIMER5B timeout
#if DEBUG
  addTInfo(pStart);
